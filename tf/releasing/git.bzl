...
def _git_changelog_impl(ctx):
    ...
    toolchain = ctx.toolchains["@rules_tf//tf/toolchains/git:git_toolchain_type"].git
...
_git_changelog = rule(
    ...
    attrs = {
        ...
        "_git_changelog": attr.label(
            default = Label("//tf/releasing:git_changelog_private"),
            cfg = "exec",
            executable = True,
            allow_files = True,
        ),
    ...
    toolchains = ["@rules_tf//tf/toolchains/git:git_toolchain_type"],
)
...
def git_changelog(name, **kwargs):
    _git_changelog(
        ...
        target_compatible_with = select({
            # Force label resolution to be rules_tf, instead of my repo.
            str(Label("//tf/toolchains/git:have_git")): [],
